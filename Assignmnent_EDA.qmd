...
-   How does survival rate/months change based on access to treatment?
    ...
    -   Exploring the relationship between survival rate/months and time from diagnosis to treatment         helps us understand the effects of delaying treatment after diagnosis.
        Investigating the quality of life patients
...
df <- read.csv("C:/data/testi.csv") # This data is a subset of our original data only inclusinf observations whose primary site was on their testicles.
...
```{r}
coxph(Surv(survival.months, testi_death) ~ race + Surg.Rad.Seq + Months.diag.to.treat + marital.status.at.diagnosis, data = df) %>% gtsummary::tbl_regression(exp = TRUE)
```
