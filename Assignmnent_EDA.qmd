...
df <- read.csv("C:/Users/fvara/Documents/testi.csv")
df[df == "Blank(s)"] <- NA
...
Creating a more proper naming convention for the variables. This creates easier readability and makes it easier to reference the variables. 
...
km.trt <- survfit(SurvObj ~ race, data = df, conf.type = "log-log")
km.race <- survfit(SurvObj ~ race, data = df, conf.type = "log-log")
...
```
```{r}
#should potentially remove some of these
coxph(Surv(survival.months, testi_death) ~ race + Scope.Reg.LN.Sur + Surg.Oth.Reg.Dis + Surg.Rad.Seq + Months.diag.to.treat + marital.status.at.diagnosis, data = df) %>% gtsummary::tbl_regression(exp = TRUE) 
```
